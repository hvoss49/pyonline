#!/bin/bash
#
# Compiles given source, running pdfpython on it couple of times
#
# Usage
...
cd $rootdir
PYTHONRUN="$(dirname $0)"/../pythonrun/pythonrun
PYTHONUNBUFFERED=true $PYTHONRUN --verbose-cmd --python-cmd=${command} -Wall -o $outputFile $target &>$logFile
