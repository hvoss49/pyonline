...
class PythonOnline extends EventEmitter {
    /**
    ...
    static async create(tmpFolder, resultsFolder) {
        ...
        if (!downloadManager || !resultsFolderSuccess)
            return null;
        return new PythonOnline(downloadManager, resultsFolder);
    }
     ...
     * @return {!Promise<!PythonOnline.Result>}
     */
    async prepareGitCompilation(url, target, branch, command, workdir) {
        ...
        if (!request)
            return new PythonOnline.Result(null, null, userError);
        var downloader = this._downloadManager.createGitDownloader(url);
        return new PythonOnline.Result(request, downloader, null);
    }
